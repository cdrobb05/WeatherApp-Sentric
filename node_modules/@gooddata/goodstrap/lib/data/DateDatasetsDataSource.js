...
exports.unrelatedHeader = exports.relatedHeader = exports.otherHeader = exports.recommendedHeader = void 0;
...
function addIndex(items) {
  ...
  return items.map(function (item) {
    if (item.type !== "header") {
      item.index = i; // eslint-disable-line no-param-reassign
...
function preselectDateDataset(dateDatasets, recommendedDate) {
  var others = dateDatasets.filter(function (d) {
    return d.id !== recommendedDate.id;
  });
...
function createDataSource(dateDatasets) {
  ...
  var indexed = addIndex(items);
  return {
    rowsCount: indexed.length,
    getObjectAt: function getObjectAt(rowIndex) {
      return indexed[rowIndex];
    }
  };
}